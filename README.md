...
## Build de producci�n (Cloudflare Tunnel)

La app tiene dos modos, decididos en **tiempo de compilaci�n**:

| Compilaci�n | URL efectiva | Fallback |
|---|---|---|
| `flutter run` / `flutter build apk --release` (sin define) | `http://100.71.116.89:5047` (Tailscale) con respaldo LAN | s� (dev) |
| `flutter build apk --release --dart-define=API_BASE_URL=https://sync.<cliente>.com` | hostname p�blico HTTPS del t�nel | **ninguno** |

Reglas del modo producci�n (`API_BASE_URL` presente):

* Solo se acepta `https://` � la compilaci�n falla al arrancar con otro valor.
* �NICA URL: no hay reintento contra IPs privadas (LAN/Tailscale quedan fuera).
* El estado de conexi�n se valida sondeando `/health` por HTTPS
  (los sockets TCP crudos no distinguen edge ca�do de origen ca�do).

El hostname se publica con Cloudflare Tunnel desde el sincronizador;
procedimiento completo y delegaci�n GoDaddy ? Cloudflare en el repo del
sincronizador: `Deploy/Cloudflare/README.md`.

> Nota de alcance: la URL se fija SOLO al compilar. El cambio din�mico de
> servidor desde la app (M�s ? Configuraci�n) es una fase posterior y exige
> redise�ar el reinicio de DioClient, ConnectionStateService, sesi�n y SQLite.
